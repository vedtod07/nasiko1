...
MAF_TEST_QUERIES_DIR = "router/data/maf_test_queries"
...
def load_maf_idea_by_filename(filename: str) -> Dict[str, Any]:
    """Load a MAF idea file by its filename."""
    maf_idea_path = Path(MAF_IDEAS_DIR) / filename
    with open(maf_idea_path, "r", encoding="utf-8") as f:
        return json.load(f)


def load_maf_test_queries(filename: str, query_set_idx: int) -> List[Dict[str, Any]]:
    """
    Load MAF test queries from a file and return the specified query set.

    Args:
        filename: The MAF test queries filename (e.g., "maf_test_queries_81.json")
        query_set_idx: Index of the query set to retrieve

    Returns:
        List of query dicts with 'query', 'target_agent', and optionally 'response'
    """
    queries_path = Path(MAF_TEST_QUERIES_DIR) / filename
    with open(queries_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    # Assumes format: {"query_sets": [[{query, target_agent, response}, ...], ...]}
    return data["query_sets"][query_set_idx]["query_response_pairs"]
...
def select_registries_by_size_ranges(
    ...
    for range_name, range_filter in SIZE_RANGES:
        ...
        indices_in_range = [
            i for i, reg in enumerate(registries) if range_filter(reg["actual_size"])
        ]
...
def get_test_cases_for_registry(
    ...
    """Get all test cases (MAF tests) for a specific registry index."""
    all_test_cases = [tc for tc in test_cases if tc["registry_id"] == registry_idx]
    sample_test_cases = random.sample(all_test_cases, min(len(all_test_cases), 5))
    return sample_test_cases


def load_maf_test_case_data(test_case: Dict[str, Any]) -> Dict[str, Any]:
    """
    Load test case data for a MAF test.

    Args:
        test_case: Test case dict with maf_file, queries_file, query_set_idx

    Returns:
        Dict with maf_name, maf_file, and queries (list of query dicts)
    """
    queries = load_maf_test_queries(
        test_case["queries_file"], test_case["query_set_idx"]
    )
    return {
        "maf_name": test_case["maf_name"],
        "maf_file": test_case["maf_file"],
        "queries": queries,
    }
...
def load_agent_cards_for_registry(registry: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Load agent cards for all agents across all MAFs in a specific registry.

    Args:
        registry: Registry dict containing mafs list with maf_file references

    Returns:
        List of agent cards for all agents in the registry
    """
    registry_agent_cards = []
    for maf_entry in registry["mafs"]:
        maf_filename = maf_entry["maf_file"]
        maf_idea = load_maf_idea_by_filename(maf_filename)
        for agent in maf_idea["agents"]:
            agent_card_filename = agent["agent_card_file"]
            agent_card = load_agent_card_by_filename(agent_card_filename)
...
def build_vectorstore_for_registry(
    ...
    """
    Build a FAISS vectorstore for all agents across all MAFs in a specific registry.
...
def semantic_search_exps(embedding_file=EMBEDDINGS_FILE):
    ...
    for registry_idx in tqdm(selected_registry_indices, desc="Processing registries"):
        ...
        registry_size = registry["actual_size"]
        ...
        # Process each test case (MAF test)
        for test_case_idx, test_case in enumerate(registry_test_cases):
            maf_name = test_case.get("maf_name", f"MAF {test_case_idx}")
            print(f"Processing MAF test '{maf_name}' for registry {registry_idx}.")
            conversation_history = []
            turn_times = []  # Track time for each turn

            shortlists_file.write(f"  MAF Test: {maf_name}\n")

            # Load queries for this MAF test
            try:
                test_case_data = load_maf_test_case_data(test_case)
                queries = test_case_data["queries"]
            except Exception as e:
                print(f"Error loading test case data for MAF '{maf_name}': {e}")
                shortlists_file.write(f"    ERROR: Could not load queries - {e}\n\n")
                continue

            print(f"  {len(queries)} queries:")
            print(f"  query 0 : {queries[0]}")
            for turn_idx, query_data in enumerate(queries):
                user_message = query_data["query"]
                correct_agent = query_data["agent_name"]
                ai_response = query_data.get("response", "")
                print(f"  Turn {turn_idx}: {user_message[:50]}...")
                total_turns += 1
                ...
                try:
                    ...
                    shortlists_file.write(f"      User Message: {user_message}\n")
